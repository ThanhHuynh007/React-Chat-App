    ...
    <div id="chat_footer" className='w-full h-20 rounded-br-xl flex items-center bg-bgLight2 dark:bg-bgDark2 border-t-2 border-gray-300 dark:border-messageHover'>
      <div className='w-full px-3 flex flex-row items-center justify-around' >
        ...
        <button>
            ...
            disabled={
              selectUser.type === "group"
                ? isThere === false || !groupStates
                  ? true
                  : false
                : filtered?.length <= 0 && find?.length <= 0 && userIsThere
                  ? false
                  : true
            }
            onChange={(e) => {
          ...
          <label htmlFor="file">
            {img === "" && <IoMdImages className="w-8 h-8 text-bgDark2 dark:text-loginInfo cursor-pointer" />}
            {img !== "" && (
              <img
                src={previewImage}
                alt="Selected Image"
                className="w-8 h-8 object-cover rounded-full shadow-2xl shadow-neutral-900 cursor-pointer"
              />
            )}
          </label>
